# Воспроизведение анализа статьи "Transcriptional response of _Saccharomyces cerevisiae_ to lactic acid enantiomers"
...
### Построение пробных графиков на основе данных об активности фенолоксидазы (PO) у _E. coli_ и _E. verrucosus_
...
В volcanoplot Log2FC (ось X) это логарифм по основанию 2 от изменения экспрессии генов. Она показывает каким образом и в какой степени изменяется экспрессия определённых генов в различных условиях (например, в экспериментальной группе по сравнению с контрольной группой). Значения выше нуля означают повышение экспрессии генов, а значения меньше нуля – понижение. Нулевое значение, соответственно означает отсутствие изменений.
p.adj это adjusted p-value, иными словами это скорректированное значение p. По оси Y отмечается отрицательный десятичный логарифм от этого значения.

### Аннотация полученных результатов

На получившемся volcanoplot`е наглядно представлены изменения в экспрессии определённых генов в экспериментальных условиях, однако, чтобы сделать выводы на основе этого, нужно знать за что эти гены отвечают. В случае с _Saccharomyces_, можно воспользоваться Sacchromyces Genome Database: [https://yeastgenome.org].
